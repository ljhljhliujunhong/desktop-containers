...
编辑模式是全局的，在管理窗口侧栏、托盘、设置里切换。管理窗口的「容器」页是左右两栏：左边点一个已有容器，右边编辑这一只的名称、图标、标题、锁定、颜色、透明度，以及里面应用的上移、下移、移出、移到别的容器。首页点预览，或在桌面容器上点「外观」，都会选中那一只并打开这一页。首页的「统一透明度」勾上以后，桌面容器的背景透明度都用这一条；各自在容器页里的透明度还留着，取消勾选就回到各自的值。不要把统一值写进容器的 `appearance.backgroundOpacity`。
